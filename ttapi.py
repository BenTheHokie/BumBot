...
class Bot:
   ...
   def updatePresTmr(self):
      self.updateTmr = threading.Timer(10, self.updatePresTmr) #Repeating updatePresence every 10s like in the NodeJS version
      self.updateTmr.start()
      self.updatePresence()
   ...
   def speak(self, msg, callback=None):
      rq = { 'api': 'room.speak', 'roomid': self.roomId, 'text': msg }
      self._send(rq, callback)


   def pm(self, msg, userid, callback=None):
      rq = { 'api': 'pm.send', 'receiverid': userid, 'text': msg }
      self._send(rq, callback)
